...
spielobject = function(arg_SpielID,arg_oBlatt,argAlleSpieler)
{
	...
	******************************************************************************/	
	...
	this.zug_machen = function(iSpielernummer, iSpielerstapel, iBankstapel){
		...
		//Prüfung der Eingabe
		if(iSpielernummer != selbst.get_aktuellen_spieler())return MOE_FEHLER_NichtAktuellerSpielerMachtZug; //throw "Der Spieler "+iSpielernummer+" hat versucht, einen Zug zu machen, obwohl er nicht dran ist. Es ist Spieler "+selbst.get_aktuellen_spieler()+" an der Reihe.";
		if( aAlleSpieler[iSpielernummer].kartenstapel(iSpielerstapel).ist_leer()) return MOE_FEHLER_SpielerZiehtVonLeeremStapel;//throw "Der Spieler "+iSpielernummer+" hat versucht, vom Stapel "+iSpielerstapel+" zu ziehen. Dort liegt aber keine Karte!";
				...
				if(!Bank.bankstapel(0).ist_leer() && !Bank.bankstapel(1).ist_leer() && !Bank.bankstapel(2).ist_leer()){
					if(doSpielerkarte.get_farbe()!=Bank.bankstapel(iBankstapel).get_offene_karte().get_farbe()){
						if(ma_dotp(aAlleSpieler[iSpielernummer].get_farben_offener_karten(),Bank.get_farben_offener_karten())>0){
							return 1201; //Fehler für Falsch bedient
						}
					}
				}
			}
