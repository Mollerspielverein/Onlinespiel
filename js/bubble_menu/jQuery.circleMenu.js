;(function($, window, document, undefined){
    ...
    CircleMenu.prototype.open = function(){
		...
		if($self.hasClass("userbox_submenu")){
			$self.children(":first-child").css({"opacity":1});
		}
        ...
        var self = this,
            ...
            set.each(function(index){
                ...
                self._timeouts.push(setTimeout(function(){
                    ...
                    vendorPrefixes($item,'transform','scale(0.5)');
                    $item.css({top:0,left:0,"opacity":0});
                }, start + Math.abs(self.options.step_in) * index));


                self._timeouts.push(setTimeout(function(){
                    $item.css({"display":"none"});
                }, /*start + Math.abs(self.options.step_in) * set.length+*/self.options.speed));
            });
            self._timeouts.push(setTimeout(function(){
    ...
    CircleMenu.prototype.select = function(index){
        ...
        if(self._state === 'open' || self._state === 'opening'){
            ...
            self.element.removeClass(pluginName+'-open');
             self.element.addClass(pluginName+'-closed');
            setTimeout(function(){self.close();self.initCss();},500);
        }
    };
    ...
    CircleMenu.prototype.initCss = function(){
		...
		$items.each(function(index) {
			$( this ).attr('style','');
			//console.log( index + ":" + $( this ).text()+"/" +$(this).children("a:first-child").prop("tagName")+"/"+$(this).children("a:first-child").height());
			$(this).css({
				/*'display': 'block',*/
				'width': self.options.item_diameter+'px',
				...
				'position': 'absolute',
				//'z-index': $(this).children("a:first-child").outerHeight(),
				'opacity': '0'
			});
            /*$(this).children("a:first-child").css({
                'margin-top': ((self.options.item_diameter - $(this).children("a:first-child").outerHeight())/2)+"px",
                'display':'block'});*/
		});
        //self.element.children('li:first-child').css({ 'display':'none'/*'z-index': 1000-self.options.depth*/});
        self.menu_items.css({
            top:0,
            left:0
        });
